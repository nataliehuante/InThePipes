...
public class PlayerController : MonoBehaviour
{
	...
	void Update()
	{
        ...
        // if caught in the tidal wave, be swept away
        if (inWave)
            SweepAndDrag();
	}
    ...
    private void listenForMovementInput() {
        ...
        if (successfulSwing && (!onFloor)) {
            jumpCount = 2;
            // track if player's first movement, will set off wave movement
            if (hasMoved == false)
                hasMoved = true;
        }
        else if (jumpCount == 2){ // in the air but not swinging
        }
        else {  // if not swinging and not in the air after swinging, listen for movement input
